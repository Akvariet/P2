...
function instantiateUser(user){
  ...
  const id    = user.id;
  const color = `rgb(${user.color[0]}, ${user.color[1]},${user.color[2]})`;
  ...
  user.pos[1] = body.style.top  = user.pos[1] + "%";
  user.pos[0] = body.style.left = user.pos[0] + "%";
...
function userMove(user) {
  ...
  function userDrag(e) {
    ...
    pos3 = e.clientX;
    pos4 = e.clientY;
    user.pos[0] = (userElement.offsetTop - pos2);
    user.pos[1] = (userElement.offsetLeft - pos1);

    userElement.style.top = user.pos[0] + "px";
    userElement.style.left = user.pos[1] + "px";

    /*USER COMMUNICATION WITH SERVER SHOULD HAPPEN HERE*/
    socket.emit('user-pos', user.pos);
  }
...
function UserContainer() {
  ...
  this.removeUser = id => this[`${id}`] = undefined;
}